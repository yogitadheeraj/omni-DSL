...
function FundamentalsSection({
  ...
  return (
    <section style={{ padding: layoutSectionPadding, borderTop: "1px solid #e2e8f0" }}>
      <div className="mb-5">
        <h2 className="text-3xl font-bold text-slate-900">Token Fundamentals Guidelines</h2>
        <p className="text-slate-600">Select a brand, then explore fundamentals like Typography, Color, Spacing, Radius, and Headings.</p>
      </div>
      ...
      {!activeTheme ? (
        ...
        <div className="grid lg:grid-cols-[240px_1fr]" style={{ gap: layoutGridGutter }}>
          <aside className="rounded-lg border border-slate-200 bg-white p-3" style={{ borderRadius: radiusSection }}>
            <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-500">Fundamentals Menu</p>
              ...
              {fundamentalMenu.map((item) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: active ? previewPrimary : "#f8fafc",
                      color: active ? previewInverse : "#0f172a",
                      borderColor: active ? previewPrimary : "#e2e8f0"
                    }}
                  >
          ...
          <article className="rounded-lg border border-slate-200 bg-white p-4" style={{ borderRadius: radiusSection }}>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h3 className="text-xl font-semibold text-slate-900">{fundamentalMenu.find((item) => item.key === selectedFundamental)?.label || "Guidelines"}</h3>
                <p className="text-sm text-slate-500">Brand: {activeBrandName}</p>
              </div>
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
                {selectedFundamentalTokens.length} tokens
              </span>
            </div>

            <div className="grid md:grid-cols-2" style={{ gap: layoutCardGutter }}>
                {selectedFundamentalTokens.map((token) => (
                  <div key={token.name} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2" style={{ borderRadius: radiusPanel }}>
                    <p className="text-xs text-slate-500">{token.name}</p>
                    <p className="text-sm font-semibold text-slate-900">{token.resolvedValue || token.value || "-"}</p>
                    <p className="text-[11px] text-slate-400">Type: {token.type}</p>
                  </div>
                ))}
              </div>
          </article>
...
console.log("Token entries:", tokenEntries, activeTokens);
  ...
  const typographyProfile = {
    fontFamilyBase: firstTokenValue(activeTokens, ["font.family.base", "font.family.primary", "typography.font.family.base"], "Inter"),
    fontFamilyHeading: firstTokenValue(activeTokens, ["font.family.heading", "typography.heading.font.family", "font.family.base"], "Inter"),
    fontWeightHeading: firstTokenValue(activeTokens, ["font.weight.heading", "typography.heading.font.weight", "font.weight.bold"], "700"),
    lineHeightHeading: firstTokenValue(activeTokens, ["line-height.heading", "typography.heading.line-height", "line-height.tight"], "1.2")
  };
